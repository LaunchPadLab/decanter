      ...
      it 'raises an argument error' do
        expect { Decanter.decanter_for(foo) }
          .to raise_error(ArgumentError,
                          "cannot lookup decanter for #{foo} with class #{foo.class}")
      end
    end
      ...
      context 'when a corresponding decanter exists' do
        ...
        context 'and the class name is a frozen string' do
          let(:foo) do
            Class.new do
              def self.name
                'Foo' # ActiveRecord classes might have a frozen name (Rails 5.2.3)
              end
            end
          end
        ...
        it 'raises a name error' do
          expect { Decanter.decanter_for(foo) }
            .to raise_error(NameError,
                            "uninitialized constant #{foo.to_s.capitalize}Decanter")
        end
      end
